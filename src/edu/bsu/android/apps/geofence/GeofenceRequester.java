...
/**
 * Class for connecting to Location Services and requesting geofences. <b> Note: Clients must ensure that Google Play services is available before
 * requesting geofences. </b> Use GooglePlayServicesUtil.isGooglePlayServicesAvailable() to check.
 * 
 * 
 * To use a GeofenceRequester, instantiate it and call AddGeofence(). Everything else is done automatically.
 * 
 */
public class GeofenceRequester implements OnAddGeofencesResultListener, ConnectionCallbacks, OnConnectionFailedListener {
	...
	/**
	 * Request a connection to Location Services. This call returns immediately, but the request is not complete until onConnected() or
	...
	public void onAddGeofencesResult(int statusCode, String[] geofenceRequestIds) {
		...
		if (LocationStatusCodes.SUCCESS == statusCode) {
			...
			FileUtils.writeFile("geofence_status.txt", "GeofenceRequester | LocationStatusCode: " + msg);
			 ...
			 */
			msg = mActivity.getString(R.string.add_geofences_result_failure, statusCode, Arrays.toString(geofenceRequestIds));
	...
	public void onConnected(Bundle arg0) {
		...
		//mLocationClient.requestLocationUpdates(mLocationRequest, new MyListener());
	...
	public void onDisconnected() {
		...
		//mLocationClient.removeLocationUpdates(new MyListener());
		// Destroy the current location client
		mLocationClient = null;
	}

	/**
	 * Get a PendingIntent to send with the request to add Geofences. Location Services issues the Intent inside this PendingIntent whenever a
	...
	private PendingIntent createRequestPendingIntent() {
		...
		if (null != mGeofencePendingIntent) {
			...
			/*
			 * Return a PendingIntent to start the IntentService. Always create a PendingIntent sent to Location Services with FLAG_UPDATE_CURRENT, so
			 * that sending the PendingIntent again updates the original. Otherwise, Location Services can't match the PendingIntent to requests made
			 * with it.
			 */
	...
	/*
	 * Implementation of OnConnectionFailedListener.onConnectionFailed If a connection or disconnection request fails, report the error
	...
	public void onConnectionFailed(ConnectionResult connectionResult) {
		...
		if (connectionResult.hasResolution()) {
			...
			/*
			 * If no resolution is available, put the error code in an error Intent and broadcast it back to the main Activity. The Activity then
			 * displays an error dialog. is out of date.
			 */
		} else {
	...
	public class MyListener implements LocationListener {
		...
		public void onLocationChanged(Location location) {
			...
			Toast.makeText(mActivity, location.getLatitude() + " : " + location.getLongitude(), Toast.LENGTH_LONG).show();
		}
	}*/
}
