...
/**
 * This class receives geofence transition events from Location Services, in the form of an Intent containing the transition type and geofence id(s)
...
public class ReceiveTransitionsIntentService extends IntentService {
	 ...
	 * 
	 * @param intent The Intent sent by Location Services. This Intent is provided to Location Services (inside a PendingIntent) when you call
	 *            addGeofences()
	 */
	@Override
		...
		if (LocationClient.hasError(intent)) {
			...
			if ((transition == Geofence.GEOFENCE_TRANSITION_ENTER) || (transition == Geofence.GEOFENCE_TRANSITION_EXIT)) {
				...
				for (int index = 0; index < geofences.size(); index++) {
					try {
						long id = Long.parseLong(geofences.get(index).getRequestId());
						SimpleGeofence item = provider.getById(id);
						
						geofenceIds[index] = item == null ? geofences.get(index).getRequestId() : item.getName();
					} catch (Exception ex) {
						FileUtils.writeFile("errors.txt", ex.getMessage());
				...
				FileUtils.writeFile("transitions.txt", getString(R.string.geofence_transition_notification_title, transitionType, ids));
	...
	/**
	 * Posts a notification in the notification bar when a transition is detected. If the user clicks the notification, control goes to the main
	 * Activity.
	 * 
